...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (Stage.curStage == "hexw" && SONG.songId.toLowerCase() == "cooling")
		{
			...
			{
				//coolingHandler = new GlobalVideo();
				//coolingHandler.play('assets/video/coolingVisualizer'), null, coolingVideo, false, false, true);
			}
			else
			{
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					if (isCooling)
					{
						//switchState(new VideoState('assets/videos/eef.webm', new PlayState()));
						switchState(new PlayState());
						clean();
					}
					else
